...
import interview.guide.infrastructure.file.ocr.DocumentOcrService;
...
public class DocumentParseService {
    ...
    public DocumentParseService(TextCleaningService textCleaningService, DocumentOcrService documentOcrService) {
    ...
    public String parseContent(MultipartFile file) {
        ...
        try (InputStream inputStream = file.getInputStream()) {
            ...
            return parseContent(fileBytes, fileName);
        } catch (IOException e) {
    ...
    public String parseContent(byte[] fileBytes, String fileName) {
        ...
        try (InputStream inputStream = new ByteArrayInputStream(fileBytes)) {
            String content = parseWithTika(inputStream);
            content = applyOcrFallbackIfNeeded(fileBytes, fileName, content);
    ...
    private String applyOcrFallbackIfNeeded(byte[] fileBytes, String fileName, String tikaText) {
        String ocrText = documentOcrService.tryExtractText(fileBytes, fileName, tikaText);
        return StringUtils.hasText(ocrText) ? ocrText : tikaText;
    }
