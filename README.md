...
**智能求职服务平台** - 基于大语言模型的简历分析、模拟面试和 RAG 知识库系统
...
智能求职服务平台是一个集成了简历分析、模拟面试（文字 + 语音）、面试安排、知识库管理和多模型配置的一站式求职辅助平台。系统利用大语言模型（LLM）、向量数据库、Redis Stream 异步任务和实时语音技术，为求职者提供智能化的简历评估、面试练习、知识库问答和面试日程管理能力。

## 系统架构

后端采用 Spring Boot + Spring AI，数据层使用 PostgreSQL（pgvector）与 Redis Stream，文件存储兼容 S3（MinIO/RustFS）；前端为 React + Vite。核心模块包括简历管理、模拟面试、面试安排、知识库 RAG 与多模型配置。
...
3. 构建工具为什么选择 Gradle？便于依赖管理、多模块扩展，并与 Spring Boot 生态集成良好。
...
- **面试中心**：文字/语音模拟面试入口，Skill 驱动出题
- **简历管理**：上传、AI 分析、历史记录与 PDF 报告导出
- **面试记录**：文字/语音面试历史、评估详情与报告导出
- **面试安排**：日历视图、邀请解析与状态管理
- **知识库**：文档上传、向量化与 RAG 问答助手
- **系统设置**：多模型 Provider、向量模型与语音服务配置
